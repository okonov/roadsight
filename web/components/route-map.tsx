"use client";

import Image from "next/image";
import { useState } from "react";
import { STATIC_MAP_SIZE } from "@/lib/maps/static-map";
import { Route } from "@/lib/routes/types";

/**
 * Static map of a resolved route's endpoints. Renders nothing when the route has no
 * endpoints yet, or when the map can't be produced (no Azure Maps key configured, service
 * down) — the review step below it still works without a picture.
 */
export function RouteMap({ route }: { route: Route }) {
  const [failed, setFailed] = useState(false);

  if (!route.origin || !route.destination || failed) return null;

  return (
    <Image
      // Endpoints can change without the URL changing, so version it on the route's own
      // timestamp; the proxy caches the image for a day.
      src={`/api/routes/${route.id}/map?v=${encodeURIComponent(route.updatedAt)}`}
      alt={`Map from ${route.origin.label} to ${route.destination.label}`}
      width={STATIC_MAP_SIZE.width}
      height={STATIC_MAP_SIZE.height}
      // Already rendered at exactly the size it is displayed at; the optimizer would only
      // re-encode a finished PNG.
      unoptimized
      onError={() => setFailed(true)}
      className="h-auto w-full rounded border border-foreground/10 bg-foreground/5"
    />
  );
}
